...
//    private JavascriptLibrary jsLib = new JavascriptLibrary();
    ...
    protected boolean atomicClick(WebElement webElement) {
        driverServices.getFluentWait().until(new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver webDriver) {
                try {
                    driverServices.getDriver().findElement(By.className("loading-spinner"));
                    return false;
                } catch (Exception ex) {
                    return true;
                }
    ...
    private void focus(WebElement webElement) {
        driverServices.executeJavaScript("arguments[0].dispatchEvent(new Event('focus'));", webElement);
//        jsLib.callEmbeddedSelenium(driverServices.getDriver(), "triggerEvent", webElement, "focus");
    }

    private void blur(WebElement webElement) {
        driverServices.executeJavaScript("arguments[0].dispatchEvent(new Event('blur'));", webElement);
